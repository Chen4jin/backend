...
public class Photo {
  ...
  public ResponseEntity<ApiResponse> getPhoto() {
    ...
    try {
      DescribeTableResponse describeTableResponse = this.connector
          .getClient()
          .describeTable(DescribeTableRequest.builder().tableName(this.tableName).build());

      Map<String, String> data = Map.of("count", String.valueOf(describeTableResponse.table().itemCount()));
      ...
      ApiResponse response = new ApiResponse("error", 500, "Failed to init DynamoDbClient", "", e.getMessage());

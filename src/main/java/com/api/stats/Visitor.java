...
public class Visitor {
  ...
  public ResponseEntity<ApiResponse> getVisitor() {
    ...
    GetItemRequest getItemRequest =
        GetItemRequest.builder().tableName(this.tableName).key(this.primaryKey()).build();
    ...
    try {
      ...
      if (getItemResponse.hasItem()) {
        ...
        response =
            new ApiResponse(
                "success",
                200,
                "No existing item was found; a new item has been successfully created.",
                data,
                null);
      }
      ...
      response =
          new ApiResponse(
              "error", 500, "AWS SDK client initialization failed.", "", e.getMessage());
  ...
  public ResponseEntity<ApiResponse> postVisitor() {
    ...
    try {
      UpdateItemResponse updateItemResponse =
          this.getConnector().getClient().updateItem(updateItemRequest);
      ...
      if (updateItemResponse.hasAttributes()) {
        ...
        response =
            new ApiResponse(
                "error", 503, "Failed to execute UpdateItem request on DynamoDB.", data, null);
        return ResponseEntity.internalServerError().body(response);
      }
